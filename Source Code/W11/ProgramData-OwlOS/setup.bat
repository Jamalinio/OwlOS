...
echo Optimizing bcdedit options
::ustawienia bcdedit
...
bcdedit /set {current} description "OwlOS 11" >nul 2>&1
...
::usuwanie reguł firewall
::źródło: https://github.com/amitxv/PC-Tuning
...
echo Optimizing fsutil behaviors
::zastosowywanie ustawień dla fsutil behaviors
::źródło: https://github.com/amitxv/PC-Tuning
...
::wyłączanie gamebar presence writer
Reg.exe add "HKLM\SOFTWARE\Microsoft\WindowsRuntime\ActivatableClassId\Windows.Gaming.GameBar.PresenceServer.Internal.PresenceWriter" /v "ActivationType" /t REG_DWORD /d "0" /f >nul 2>nul

echo Disabling memory compression
::wyłączanie kompresji ramu (ma to wpływ na większe zużycie pamięci ram przez aplikacje etc, ale przyśpiesza ich uruchamianie)
PowerShell "Disable-MMAgent -MemoryCompression" >nul 2>&1

echo Disabling scheduled tasks
::wyłączanie zaplanowanych zadań
...
::zastosowywanie niestandardowego planu zasilania
...
echo Disabling services limiting power
::wyłączanie usług ograniczających energie
::źródło: https://github.com/amitxv/PC-Tuning
for %%a in (
...
echo Disabling remote assistance functions
::wyłączanie funkcji zdalnej pomocy
::źródło: https://github.com/amitxv/PC-Tuning
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v "UserAuthentication" /t REG_DWORD /d "0" /f >nul 2>&1

echo Disabling spell checking functions
::wyłączanie funkcji sprawdzania pisowni
::źródło: https://github.com/amitxv/PC-Tuning
...
echo Disabling language bar shortcut key
::wyłączenie klawisza skrótu paska językowego (ctrl + shift)
...
::włączanie przeglądarki zdjęć (win7)
